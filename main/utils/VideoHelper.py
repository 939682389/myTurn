...
class mp4_to_H264():
    ...
    def convert_avi(self, input_file, output_file, ffmpeg_exec="ffmpeg"):
        ffmpeg = '{ffmpeg} -y -i "{infile}" -c:v1 libx264 -strict -2 "{outfile}"'.format(ffmpeg=ffmpeg_exec,
                                                                                        infile=input_file,
                                                                                        outfile=output_file)
        f = os.popen(ffmpeg)
...
a = mp4_to_H264()
from_path = 'main/templates/video/1580367763.mp4'
to_path = 'main/templates/video/1580367763.mp4'
